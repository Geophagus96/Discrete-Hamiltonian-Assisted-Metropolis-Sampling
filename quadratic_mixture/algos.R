...
#########################################################
## Codes for "Discrete Hamiltonian-Assisted Metropolis Sampling" for quadratic mixtures
#########################################################
...
#
#--- gradient_ene: the function that calculates f(s) and gradient of f(s)
#         Arguments: mus, list of means for each mixture; Ws, list of variance for each mixture

gradient_ene <- function(x, mus, Ws) {
  components <- mapply(function(mu, W) {
    diff <- x - mu
    exp(-0.5 * t(diff) %*% W %*% diff)/29
  }, mus, Ws, SIMPLIFY = TRUE)
  
  density_sum <- sum(components)
  log_density <- log(density_sum)
  
  components = components/density_sum
  gradients <- mapply(function(mu, W, comp) {
    -comp * W %*% (x - mu)  
  }, mus, Ws, components, SIMPLIFY = FALSE)
  
  gradient_sum <- Reduce(`+`, gradients)
  
  list(ex = as.numeric(log_density),
       gx = as.vector(gradient_sum))
}
...
MH_uniform = function(n, param_list, seeds){
  ...
  for(i in 1:n){
    ...
    if((-10 <= xnew[entry]) & (xnew[entry] <= 10)){
        new_info = gradient_ene(xnew, mus, Ws)
        new_energy = new_info$ex
  ...
  return(list(xs = xs, accs=accs, fvals = fvals))
}
...
DHAMS =  function(n, param_list, seeds){
  ...
  u = rnorm(ndim)
  ngrids = matrix(rep(grid, ndim), ndim , length(grid), byrow=TRUE)
  old_info = gradient_ene(s, mus, Ws)
  old_energy = old_info$ex
  oldgrad = old_info$gx
  for(i in 1:n){
    ...
    new_info = gradient_ene(snew, mus, Ws)
    newgrad = new_info$gx
    new_energy = new_info$ex    
    ...
    log_ratio = new_energy-old_energy-0.5*(sum(unew^2)-sum(u12^2))
  ...
  return(list(xs=xs, us=us, accs=accs, fvals = fvals))
}
